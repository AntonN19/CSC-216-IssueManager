...
/**
 * Container for the IssueManager that has the menu options for new issue files,
 * loading existing files, saving files and quitting. Depending on user actions,
 * other JPanels are loaded for the different ways users interact with the UI.
 * 
...
public class IssueManagerGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a IssueManagerGUI object that will contain a JMenuBar and a JPanel
	 * that will hold different possible views of the data in the IssueManager.
	 */
	...
	private class IssueListPanel extends JPanel implements ActionListener {
			...
			if (e.getSource() == btnAddNewIssue) {
				...
				if (row == -1) {
					...
					try {
						...
						JOptionPane.showMessageDialog(IssueManagerGUI.this, "Invalid issue id");
					}
				}
				updateTable(null);
				...
				if (row == -1) {
					...
					try {
						int issueId = Integer.parseInt(issueTableModel.getValueAt(row, 0).toString());
						String stateName = IssueManager.getInstance().getIssueById(issueId).getState();
		...
		/**
		 * IssueTableModel is the object underlying the JTable object that displays the
		 * list of Issues to the user.
		 * 
		 * @author Dr. Sarah Heckman (sarah_heckman@ncsu.edu)
		 */
		private class IssueTableModel extends AbstractTableModel {
			...
			/**
			 * Constructs the IssueTableModel by requesting the latest information from the
			 * IssueTableModel.
			 */
			...
			/**
			 * Updates the given model with Issue information for the given issue type from
			 * the IssueManager.
			 * 
			...
			private void updateIssueDataByType(IssueType issueType) {
				try {
					...
					JOptionPane.showMessageDialog(IssueManagerGUI.this, "Invalid issue type");
				}
			}
		}
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAssign) {
				...
				if (ownerId == null || ownerId.equals("")) {
					...
					JOptionPane.showMessageDialog(IssueManagerGUI.this, "Invalid developer id");
					reset = false;
				} else {
					...
					try {
						...
						JOptionPane.showMessageDialog(IssueManagerGUI.this, "Invalid command");
						reset = false;
					} catch (UnsupportedOperationException uoe) {
						JOptionPane.showMessageDialog(IssueManagerGUI.this, "Invalid state transition");
						reset = false;
					}
				}
				...
				if (idx == -1) {
					reset = false;
					JOptionPane.showMessageDialog(IssueManagerGUI.this, "No resolution selected");
					reset = false;
				} else {
					Resolution r = null;
					switch (idx) {
	...
	private class AddIssuePanel extends JPanel implements ActionListener {
		...
		/**
		 * Creates the JPanel for adding new issues to the manager.
		 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAdd) {
				...
				if (idx == -1) {
					reset = false;
					JOptionPane.showMessageDialog(IssueManagerGUI.this, "No issue type selected");
				} else {
					IssueType type = null;
					switch (idx) {
					case 0:
